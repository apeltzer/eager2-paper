...
<small><em>
This manuscript
([permalink](https://apeltzer.github.io/eager2-paper/v/31c1d20b022de42ce70f22ec8c2402c75a5c97f3/))
was automatically generated
from [apeltzer/eager2-paper@31c1d20](https://github.com/apeltzer/eager2-paper/tree/31c1d20b022de42ce70f22ec8c2402c75a5c97f3)
on October 12, 2020.
</em></small>
...
GUI format across multiple pipelines as well as additional robusticity checks of
...
command as a single parameter (as with EAGER v1), however integration with
...
by utilising Nextflow's ability to customise resource settings the parameters
...
estimation of human _nuclear_ contamination estimation is now offered through
...
reference genome, nf-core/eager can now also can give estimates of the
biological sex of a given individual with Sex.DetERRmine
...
optional use as input of a simple table in TSV format (a format more commonly
used in wet-lab stages of data generation than PALEOMIX's yaml format), which
...
re-analysed data from Barquera et al. [@doi:10.1016/j.cub.2020.04.002], who
...
yaws, and a second individual (Lab ID: SJN001) with the presence of the
...
Metagenomic hits to both pathogens from the same individuals that yielded
...
virus reads. Other taxa in HOPS output represent typical oral commensal
microbiota found in teeth. MultiQC v1.10 will be included in a point release of
nf-core/eager, once released by the developer, but in the meantime can be
manually run to get the same results as v1.9 including the integrated HOPS
heatmap. Data was shotgun data from Barquera et al. 2020
...
for both MultiQC 1.9 and 1.10 can be seen in the supplementary
...
from the output of the GNU time tool. {#tbl:benchmarking-runtimes}.
...
library, whereas nf-core/eager only gave 4 (by default) for a single bwa process
...
we optimised parameters that were used for bwa multi-threading and multiple
lanes to the same number of bwa threads as the optimised PALEOMIX run,
...
processes](images/merging_files.png){#fig:merging-files width="70%"}
...
material, we replicated the results of Barquera et al.
...
containers for software environments, whereas for used PALEOMIX (v1.2.14) we
...
18.04 operating system (Kernel 4.15.0-112). Resource parameters of each tool
were only modified to specify the maximum available on the server and otherwise
left as default.
...
steps. 'qf' stands for mapping-quality filtered reads, calculated from the
output of the GNU time tool. All values represent mean and standard deviation
across 10 replicates of each pipeline.  {#tbl:benchmarking-results}.
