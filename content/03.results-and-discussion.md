...
'endogenous DNA' calculator (endorS.py) is also included to provide an estimate 
of the percentage of the total sequenced reads per library that map to the provided
reference sequence.

Given the large amount of sequencing often required to yield sufficient genome
coverage from aDNA data, palaeogeneticists tend to use multiple (often differently
...
including file paths and additional metadata such as sample name, library
...
providing practical guidelines on how to evaluate the characteristics of aDNA 
and their effects on downstream analyses.
...
also allow for open-source development, while ensuring the high quality of community contributions to the pipeline. While Nextflow
