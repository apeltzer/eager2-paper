...
resulted in a much faster run-time that of default nf-core/eager, due to the
approach of paleomix of mapping each lane of a library separately, whereas
...
files to account for this. Finally, when we optimised nf-core/eager to the same
number of bwa threads as the optimised paleomix run, nf-core/eager still
displayed shorter runtimes.  All metrics including mapped reads, percentage
