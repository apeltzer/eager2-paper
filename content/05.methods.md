...
Users can customize this infrastructure profile once either alone or with the nf-core community, or with their local IT to make sure that the pipeline runs successfully and can then rely on the Nextflow and nf-core framework to ensure compatibility upon further infrastructure changes. In order to run the nf-core/eager pipeline at the Max Planck Institute for the science of human history in Jena, users only have to run:

```bash
nextflow run nf-core/eager -r 2.1.0 -profile shh_cdag,test
```

This runs the testing profile of the nf-core/eager pipeline on the HPC system at the MPI SHH in Jena/Germany. Similar configs for multiple institutions already exist, so users might not even have to write their own once.
...
Users can either monitor their pipeline execution on the commandline or utilize projects such as [Nextflow Tower](https://tower.nf) to monitor their analysis pipeline during runtime.

#### Output

The pipeline produces several dozen output files in various file formats, with a more detailed listing available in the user documentation. This includes metrics, statistical analysis data and standardized output files (BAM, VCF) for close inspection and further downstream analysis, as well as a MultiQC report. The latter can even be emailed to users automatically, when starting the pipeline with a dedicated `--email you@yourdomain.org`.
